...
const parseRgbColor = (colorString: string | null): RGB | null => {
...
const HeroSection = ({
  ...
  useEffect(() => {
    ...
    const animateLoop = () => {
      if (ctx && canvas) {
        ...
        if (heroVisibleRef.current) drawArrow();
      }
    ...
    if (heroRootRef.current) {
      ...
      observer.observe(heroRootRef.current);
    }
  ...
  return (
    <div
      ref={heroRootRef}
      className="bg-background text-foreground min-h-screen flex flex-col pt-16 md:pt-20"
    >
      <main className="flex-grow flex flex-col items-center justify-center">
        ...
        <div className="mt-12 lg:mt-16 w-full max-w-5xl mx-auto overflow-hidden px-4 sm:px-6">
          <div className="bg-border rounded-[2rem] p-[0.25rem]">
            <div className="relative h-80 sm:h-96 md:h-[32rem] lg:h-[36rem] rounded-[1.75rem] bg-card flex items-center justify-center overflow-hidden">
              {imageUrl && (
